...
class DiscoveryTester(unittest.TestCase):  
    ...
    def test_stem1(self):
       self.assertEqual(ds.stem("AAA/BBB.txt"), "BBB")

    def test_stem2(self):
       self.assertEqual(ds.stem("AAA.txt"), "AAA")

    def test_extension1(self):
       self.assertEqual(ds.extension("AAA/BBB.txt"), ".txt")

    def test_extension2(self):
       self.assertEqual(ds.extension("AAA.txt"), ".txt")

    def test_filter_test_casesA(self):
        tcases=ds.filter_test_cases(["A.in", "A.out"], ds.DEFAULT_ENDINGS, ds.DEFAULT_NEEDED_FILES)
        self.assertEqual(len(tcases), 1)
        self.assertEqual(tcases["A"], {ex.INPUT_FILE : "A.in", 
    ...
    def test_filter_test_casesB(self):
        ...
        self.assertEqual(len(tcases), 1)
        self.assertEqual(tcases["B"], {ex.INPUT_FILE : "B.in", 
    ...
    def test_discover_test_casesABNoC(self):
        ...
        self.assertEqual(len(tcases), 2)
        self.assertEqual(tcases["B"], {ex.INPUT_FILE : os.path.join(folder, "B.in"), 
                                        ex.STDOUT_FILE: os.path.join(folder, "B.out"),
                                        ex.STDERR_FILE: os.path.join(folder, "B.err")})
        self.assertEqual(tcases["A"], {ex.INPUT_FILE : os.path.join(folder, "A.in"), 
